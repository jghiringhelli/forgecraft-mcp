...
function parseArgs(): { condition: string; materialize: boolean; skipMigrate: boolean } {
  ...
  if (!condition || !["naive", "control", "treatment", "treatment-v2", "treatment-v3", "treatment-v4", "treatment-v5"].includes(condition)) {
    console.error(
      "Usage: npx tsx run-tests.ts --condition naive|control|treatment|treatment-v2|treatment-v3|treatment-v4|treatment-v5 [--materialize] [--skip-migrate]"
    );
    process.exit(2);
  }
  return {
...
function resolveDbUrl(condition: string): string {
  const envKey = condition === "control"
    ...
    : condition === "treatment-v4"
    ? "DATABASE_URL_TREATMENT_V4"
    : condition === "treatment-v5"
    ? "DATABASE_URL_TREATMENT_V5"
    : "DATABASE_URL_NAIVE";
  ...
  if (!url) {
    const dockerUrl = condition === "control"
      ...
      : condition === "treatment-v4"
      ? "postgresql://conduit:conduit@localhost:5443/conduit_treatment_v4"
      : condition === "treatment-v5"
      ? "postgresql://conduit:conduit@localhost:5445/conduit_treatment_v5"
