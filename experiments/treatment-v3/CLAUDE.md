...
<!-- Conduit RealWorld API — GS Treatment-v3 Condition (dependency-registry block) -->
<!-- Extends treatment-v2: adds approved-packages.md P1 requirement + audit-before-add enforcement -->
...
## Project Identity
- **Repo**: github.com/jghiginghelli/forgecraft-mcp (experiment: `experiments/treatment-v3/output/`)
...
In your **first response** (before any feature implementation), emit the following files as
fenced code blocks. These are infrastructure files — if not emitted in P1, they will never
exist in the project:

1. **`prisma/schema.prisma`** — complete schema for all Conduit entities
2. **`package.json`** — with husky + commitlint in devDependencies and `prepare` script
3. **`.husky/pre-commit`** — tsc + lint + `npm audit --audit-level=high` + test gate (in that order; any failure aborts commit)
4. **`.husky/commit-msg`** — commitlint gate
5. **`commitlint.config.js`** — conventional commits config
6. **`.github/workflows/ci.yml`** — full CI with mutation gate (`npx stryker run`) AND `npm audit --audit-level=high` as a required step
7. **`CHANGELOG.md`** — initial file with `## Unreleased` block
8. **`docs/adrs/ADR-0001-stack.md`** stub (if not already provided in context)
9. Repository interfaces: `src/repositories/IUserRepository.ts`,
   `src/repositories/IArticleRepository.ts`, `src/repositories/ICommentRepository.ts`,
   `src/repositories/IProfileRepository.ts`
10. **`docs/approved-packages.md`** — the approved-package registry (see Dependency Registry section below)

A file referenced in documentation but not emitted as a code block **does not exist**.

## Dependency Registry

`docs/approved-packages.md` is a **living GS artifact** — emit it in P1 and maintain it throughout the project. Its purpose is to ensure every dependency is audited before it reaches `package.json`.
...
- **Dependency Inversion**: Depend on abstractions. Concrete classes are injected, never instantiated
  ...
  **Emit these interfaces in the first response** alongside the schema — a service that
  imports a concrete class cannot be unit-tested without a real database.
...
**3. Composable** — Services receive `PrismaClient` (or a repository abstraction) via constructor injection.
...
- bcrypt rounds: named constant (e.g. `BCRYPT_ROUNDS = 12`)
- JWT expiry: named constant (e.g. `JWT_EXPIRY = '30d'`)
- Pagination defaults: named constants
- All secrets: from `process.env` only
*Fix: extract to named constant at module top.*
...
A feature is not done until all 5 checks pass. Fix any violations before ending the response.
...
Commit hooks and the CI pipeline must be **emitted as fenced code blocks** in the first
response — not described in prose or README text. A hook that exists only in documentation
provides zero enforcement. If the file is not written to disk, the gate does not exist.

Emit in first response:
...
Line coverage at 80% can coexist with 58% mutation score; the mutation gate catches the gap.
