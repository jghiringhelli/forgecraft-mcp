 ...
 */
function resolveTagsFromConfig(dir: string, flagTags: string[] | undefined): Tag[] | undefined {
...
function printResult(result: { content: Array<{ type: string; text: string }> }): void {
...
async function cmdScaffold(pos: string[], flags: Flags): Promise<void> {
  ...
  const result = await scaffoldProjectHandler({
    ...
    language: (str(flags, "language") as "typescript" | "python") ?? "typescript",
    dry_run: bool(flags, "dry-run", false),
...
async function cmdReview(pos: string[], flags: Flags): Promise<void> {
  ...
  const result = await reviewProjectHandler({
    tags,
    scope: (str(flags, "scope") as "comprehensive" | "focused") ?? "comprehensive",
  });
  printResult(result);
}
...
export async function runCli(argv: string[]): Promise<boolean> {
  ...
  try {
    switch (command) {
      case "setup":    await cmdSetup(positional, flags); break;
      case "refresh":  await cmdRefresh(positional, flags); break;
      case "audit":    await cmdAudit(positional, flags); break;
      case "scaffold": await cmdScaffold(positional, flags); break;
      case "review":   await cmdReview(positional, flags); break;
      case "list":     await cmdList(positional, flags); break;
      case "classify": await cmdClassify(positional, flags); break;
