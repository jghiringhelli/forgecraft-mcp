...
export function analyzeRootPackageJson(
  ...
  if (composePath) {
    try {
      ...
      if (/postgres|mysql|mongodb|mongo/.test(composeContent)) tags.add("DATABASE");
    } catch { /* skip */ }
  }
}
...
export function analyzePythonProject(
  ...
  if (existsSync(requirementsPath)) {
    try {
      ...
      if (reqContent.includes("fastapi")) { tags.add("API"); apiSignals.push("fastapi dependency (requirements.txt)"); }
      if (reqContent.includes("click") || reqContent.includes("typer")) { tags.add("CLI"); cliSignals.push("click/typer dependency (requirements.txt)"); }
      const PY_DATABASE_DEPS = ["sqlalchemy", "psycopg2", "pymongo", "databases", "tortoise-orm"];
      if (PY_DATABASE_DEPS.some((dep) => reqContent.includes(dep))) tags.add("DATABASE");
      const PY_AUTH_DEPS = ["python-jose", "passlib", "authlib", "pyjwt"];
      if (PY_AUTH_DEPS.some((dep) => reqContent.includes(dep))) tags.add("AUTH");
    } catch { /* skip */ }
  }
  ...
  if (existsSync(pyprojectPath)) {
    try {
      ...
      if (pyprojectContent.includes("fastapi")) { tags.add("API"); apiSignals.push("fastapi dependency (pyproject.toml)"); }
      if (pyprojectContent.includes("click") || pyprojectContent.includes("typer")) { tags.add("CLI"); cliSignals.push("click/typer dependency (pyproject.toml)"); }
      const PY_DATABASE_DEPS_TOML = ["sqlalchemy", "psycopg2", "pymongo", "databases", "tortoise-orm"];
      if (PY_DATABASE_DEPS_TOML.some((dep) => pyprojectContent.includes(dep))) tags.add("DATABASE");
    } catch { /* skip */ }
  }
}
...
export function buildTagAmbiguities(
  ...
  if (hasPythonBuild && hasTypeScriptSrc) {
    ambiguities.push({
      ...
      interpretations: [
        { label: "A", description: "Python project with TypeScript frontend/tooling (primary: Python)", consequence: "Python-oriented gates applied; TypeScript treated as build tooling" },
        { label: "B", description: "TypeScript project with Python scripts/utilities (primary: TypeScript)", consequence: "TypeScript gates applied; Python treated as supplementary tooling" },
      ],
    });
  }
  ...
  if (tags.has("CLI") && tags.has("API")) {
    ambiguities.push({
      ...
      interpretations: [
        { label: "A", description: "Primarily a CLI tool (tag: CLI)", consequence: "CLI cascade applied; API-specific gates optional" },
        { label: "B", description: "Primarily an HTTP API (tag: API)", consequence: "API cascade applied; all endpoint contracts required" },
        { label: "C", description: "Hybrid CLI+API project (tags: CLI, API)", consequence: "Most restrictive cascade applied; all steps required" },
      ],
    });
    ...
    ambiguities.push({
      ...
      interpretations: [
        { label: "A", description: "Primarily a CLI tool that ships an executable (tag: CLI)", consequence: "CLI cascade applied; library-specific contracts optional" },
        { label: "B", description: "Primarily a reusable library that includes a CLI (tag: LIBRARY)", consequence: "Library cascade applied; public API contracts and versioning required" },
        { label: "C", description: "Dual-purpose CLI+library package (tags: CLI, LIBRARY)", consequence: "Most restrictive cascade applied; all steps required" },
      ],
    });
  }
 ...
 */
async function loadForgecraftYamlTags(projectDir: string, tags: Set<string>): Promise<void> {
...
export async function inferTagsFromDirectory(
  ...
  if (!hasBuildSystem) {
    if (hasMarkdownFiles(projectDir)) {
      ...
      ambiguities.push({
        ...
        interpretations: [
          { label: "A", description: "Design specification project (tag: DOCS)", consequence: "All implementation gates skipped; only spec completeness checked" },
          { label: "B", description: "Early-stage software project (no build system set up yet)", consequence: "Full cascade applied; implementation gates enforced" },
        ],
      });
    }
  }

  analyzeRootPackageJson(projectDir, tags, cliSignals, apiSignals, librarySignals);
  ...
  const hasTypeScriptSrc =
    ...
    (existsSync(join(projectDir, "tsconfig.json")) || existsSync(join(projectDir, "src", "index.ts")));

  const tagAmbiguities = buildTagAmbiguities(tags, cliSignals, apiSignals, librarySignals, hasPythonBuild, hasTypeScriptSrc, projectDir);
