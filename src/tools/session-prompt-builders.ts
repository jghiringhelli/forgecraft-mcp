...
/**
 * Scan spec artifact files for unresolved [NEEDS CLARIFICATION] markers.
 * Returns a flat list of { file, marker } pairs.
 */
export function detectClarificationMarkers(
  projectDir: string,
): Array<{ file: string; marker: string }> {
  const candidates = ["docs/use-cases.md", "docs/PRD.md", "docs/TechSpec.md"];
  ...
  for (const rel of candidates) {
    ...
    try {
      ...
      for (const m of content.matchAll(CLARIFICATION_RE)) {
        results.push({ file: rel, marker: m[0] });
      }
    } catch {
  ...
  if (existsSync(adrsDir)) {
    try {
      for (const f of readdirSync(adrsDir).filter((n) => n.endsWith(".md"))) {
        const content = readFileSync(join(adrsDir, f), "utf-8");
        for (const m of content.matchAll(CLARIFICATION_RE)) {
          results.push({ file: `docs/adrs/${f}`, marker: m[0] });
        }
      }
    } catch {
      /* skip */
...
/**
 * Build a warning block for unresolved [NEEDS CLARIFICATION] markers.
 * Returns empty string if no markers found.
 */
export function buildClarificationWarning(
  markers: Array<{ file: string; marker: string }>,
): string {
  if (markers.length === 0) return "";
  const lines = [
    `## ⚠️ Unresolved Clarifications — Session May Drift`,
    ``,
    `The following spec artifacts contain \`[NEEDS CLARIFICATION]\` markers.`,
    `**Resolve these before implementing** — the AI cannot act on ambiguity it cannot see.`,
    ``,
    `| File | Marker |`,
    `|---|---|`,
  ];
  ...
  lines.push(
    ``,
    `Edit each file and replace the marker with the actual decision.`,
    ``,
  );
  return lines.join("\n");
}
...
export function buildPrompt(input: PromptBuildInput): string {
  ...
  const clarificationMarkers = detectClarificationMarkers(projectDir);
  const clarificationWarning = buildClarificationWarning(clarificationMarkers);

  let prompt = `# Session Prompt — Bound\n\n> Generated by ForgeCraft \`generate_session_prompt\`. Load context, then issue this prompt.\n\n---\n\n`;
  if (clarificationWarning) prompt += clarificationWarning + "---\n\n";
