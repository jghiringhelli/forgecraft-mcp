...
export function writeForgeYaml(
  ...
  if (existsSync(yamlPath)) {
    try { config = yaml.load(readFileSync(yamlPath, "utf-8")) as Record<string, unknown>; }
    catch { config = {}; }
  } else {
...
function buildPrdContent(projectName: string, aiFields: AiExtractedFields): string {
  const fill = (placeholder: string) => `<!-- FILL: ${placeholder} -->`;
  const listOrFill = (csv: string | undefined, placeholder: string) =>
    csv ? csv.split(",").map((s) => `- ${s.trim()}`).join("\n") : fill(placeholder);
  return [
    `# ${projectName}\n`,
...
function buildUseCasesContent(projectName: string, aiFields: AiExtractedFields): string {
  const fill = (placeholder: string) => `<!-- FILL: ${placeholder} -->`;
 ...
 */
export function loadExistingCascadeDecisions(projectDir: string): CascadeDecision[] | null {
  const yamlPath = join(projectDir, "forgecraft.yaml");
