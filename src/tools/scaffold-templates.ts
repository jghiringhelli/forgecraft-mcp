...
/**
 * Render a per-environment smoke-test stub.
 *
 * Targets the environment's declared URL (via an env var so the same script
 * runs against any tier) and asserts the health endpoint plus a placeholder
 * for spec-derived critical paths. Tech-agnostic: a curl-based runner that the
 * AI assistant fills in, or replaces with the declared smokeTool (hurl/newman).
 *
 * @param envName    - Environment name (e.g. "staging", "prd")
 * @param env        - The environment's deployment config
 * @param deployment - Full deployment config (for smokeTool)
 * @returns Smoke-test stub script content
 */
export function renderEnvSmokeTest(
  envName: string,
  env: DeploymentEnvironmentConfig,
  deployment: ProjectDeploymentConfig,
): string {
  const health = env.health ?? "/health";
  const tier = env.class ?? "unclassified";
  const smokeTool = deployment.testing?.smokeTool;
  const urlVar = `SMOKE_URL_${envName.toUpperCase().replace(/[^A-Z0-9]/g, "_")}`;
  const defaultUrl = env.url ?? "";
  const toolNote = smokeTool
    ? `# Configured smokeTool: ${smokeTool} — replace the curl block below with a ${smokeTool} suite.`
    : `# No smokeTool set in forgecraft.yaml deployment.testing.smokeTool — using curl assertions.`;
  return `#!/usr/bin/env bash
# Smoke test — environment: ${envName} (tier: ${tier})
# Generated by ForgeCraft scaffold. Fill in critical paths from your tech spec.
# Must complete in < 60s total. Results: .forgecraft/reports/smoke-${envName}.json
${toolNote}
set -euo pipefail

# URL is read from the environment so this script is tier-agnostic.
# Default is the URL declared in forgecraft.yaml for this environment.
URL="\${${urlVar}:-${defaultUrl}}"
if [ -z "$URL" ]; then
  echo "FAIL: ${urlVar} is not set and no url declared for '${envName}' in forgecraft.yaml"
  exit 1
fi

if ! command -v curl &>/dev/null; then
  echo "FAIL: curl not found — cannot run smoke test"
  exit 1
fi
...
if [ "$STATUS" != "200" ]; then
  echo "FAIL: ${envName} health (${health}) returned $STATUS (expected 200)"
  exit 1
fi
echo "PASS: ${envName} health endpoint OK"

# ── 2. Critical user journeys ───────────────────────────────────────
# TODO: add one assertion per critical path from docs/ (use-cases / tech spec).
#   Example:
#   STATUS=$(curl -s -o /dev/null -w "%{http_code}" "$URL/api/articles")
#   [ "$STATUS" = "200" ] || { echo "FAIL: list articles -> $STATUS"; exit 1; }

echo "PASS: ${envName} smoke suite complete"
`;
}

/**
 * Render a per-environment load-test stub (k6-style, parameterized by the
 * declared thresholds). Skipped for production tier by the scaffold writer —
 * production is not a load-test target.
 *
 * @param envName    - Environment name
 * @param env        - The environment's deployment config
 * @param deployment - Full deployment config (for load thresholds)
 * @returns Load-test stub content
 */
export function renderEnvLoadTest(
  envName: string,
  env: DeploymentEnvironmentConfig,
  deployment: ProjectDeploymentConfig,
): string {
  const load = deployment.testing?.load;
  const vus = load?.concurrentUsers ?? "__TODO_concurrentUsers__";
  const rps = load?.targetRps;
  const p99 = load?.p99CeilingMs ?? "__TODO_p99CeilingMs__";
  const duration = load?.durationSeconds ?? 600;
  const urlVar = `LOAD_URL_${envName.toUpperCase().replace(/[^A-Z0-9]/g, "_")}`;
  const defaultUrl = env.url ?? "";
  const health = env.health ?? "/health";
  const rpsLine =
    rps !== undefined
      ? `// Target RPS (stated before the run): ${rps}`
      : `// Target RPS: __TODO_targetRps__ (state it in forgecraft.yaml before running)`;
  // k6 thresholds block: p99 latency ceiling is the pass/fail line.
  return `// Load test — environment: ${envName} (tier: ${env.class ?? "unclassified"})
// Generated by ForgeCraft scaffold. Tool: ${load?.tool ?? "k6 (default — set deployment.testing.load.tool)"}
// Parameters are STATED here before the run, per the GS protocol — not discovered after.
// Results: .forgecraft/reports/load-${envName}.json
${rpsLine}
import http from 'k6/http';
import { check } from 'k6';

const URL = __ENV.${urlVar} || ${JSON.stringify(defaultUrl)};

export const options = {
  vus: ${typeof vus === "number" ? vus : `0 /* ${vus} */`},
  duration: '${duration}s',
  thresholds: {
    // p99 latency ceiling — the stated NFR. Breaching it fails the run.
    http_req_duration: ['p(99)<${typeof p99 === "number" ? p99 : 0}'${typeof p99 === "number" ? "" : ` /* ${p99} */`}],
    http_req_failed: ['rate<0.01'],
  },
};

export default function () {
  if (!URL) { throw new Error('${urlVar} not set and no url declared for ${envName}'); }
  // TODO: replace with the critical-path mix from your tech spec.
  const res = http.get(URL + ${JSON.stringify(health)});
