...
export function buildCoreMdContent(
  ...
  const layerMap = buildLayerMapForTags(tags);
  const folderStructure = buildFolderStructureForTags(tags);
  return [
    `# ${projectName} — Core`,
    ``,
...
// ── Core.md tag-specific helpers ─────────────────────────────────────

/**
 * Build the layer map line for core.md based on project tags.
 *
 * @param tags - Effective project tags
 * @returns Layer map string
 */
function buildLayerMapForTags(tags: readonly string[]): string {
  if (tags.includes("WEB-NEXT") || tags.includes("WEB-REACT")) {
    return `[UI] → [API Routes] → [Services] → [Domain] → [Repositories] → [Infrastructure]`;
  }
  ...
  return `[API/CLI] → [Services] → [Domain] → [Repositories] → [Infrastructure]`;
}

/**
 * Build folder structure lines for core.md based on project tags.
 *
 * @param tags - Effective project tags
 * @returns Array of folder description lines
 */
function buildFolderStructureForTags(tags: readonly string[]): string[] {
  if (tags.includes("WEB-NEXT") || tags.includes("WEB-REACT")) {
    return [
      `src/app/          — routes (thin: validate + delegate)`,
      `src/components/   — atoms/ molecules/ organisms/`,
      `src/lib/          — services, repositories, adapters per domain`,
  ...
  if (tags.includes("CLI")) {
    return [
      `src/commands/     — CLI entry points (parse + delegate)`,
      `src/services/     — business logic`,
      `src/domain/       — domain models`,
      `src/adapters/     — filesystem, external APIs`,
      `tests/            — mirrors src/ structure`,
      `docs/             — PRD, use-cases, architecture, ADRs`,
      `.claude/          — core.md, index.md, standards/, hooks/`,
    ];
  ...
  return [
    `src/              — source (organized by domain)`,
    `tests/            — mirrors src/ structure`,
    `docs/             — PRD, use-cases, architecture, ADRs`,
    `.claude/          — core.md (always loaded), index.md, standards/, hooks/`,
  ];
}
