...
export async function generateAdrHandler(
  ...
  const lines = [
    ...
    "```",
    ``,
    `> **Next step:** If any sections contain placeholders (\`[TODO]\`), complete them before committing.`,
...
function renderAdr(fields: AdrFields): string {
  ...
  const alternativeBlock =
    alternatives && alternatives.length > 0
      ...
      : "- [TODO: list alternatives considered and why each was rejected]";
  ...
  return [
    ...
    ``,
    context ??
      "[TODO: describe the situation that forced this decision — constraints, requirements, and forces at play]",
    ``,
    `## Decision`,
    ``,
    decision ??
      "[TODO: state what was decided and why this option was chosen over the alternatives]",
    ``,
    ...
    ``,
    consequences ??
      "[TODO: describe what becomes easier, harder, or newly constrained as a result of this decision]",
    ``,
    `---`,
    ``,
