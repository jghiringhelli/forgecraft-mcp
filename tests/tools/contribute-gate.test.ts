...
describe("contributeGates", () => {
  ...
  it("skips already-submitted gates", async () => {
    ...
    writeFileSync(
      ...
      JSON.stringify([{ gateId: "gate-already-done", mode: "anonymous", status: "submitted" }]),
      "utf-8"
    );
  ...
  it("with dryRun:true returns pending without calling fetch", async () => {
    ...
    const result = await contributeGates({ projectRoot: tempDir, dryRun: true });
  ...
  it("records pending to .forgecraft/pending-contributions.json on server failure", async () => {
    ...
    const pendingData = JSON.parse(readFileSync(result.pendingFile!, "utf-8")) as unknown[];
  ...
  it("records submission to .forgecraft/contributions.json on success", async () => {
    vi.mocked(fetch).mockResolvedValue({
      ok: true,
      status: 200,
      json: async () => ({ status: "submitted", issueUrl: "https://github.com/org/repo/issues/1" }),
    } as Response);
    ...
    expect(result.submitted[0].issueUrl).toBe("https://github.com/org/repo/issues/1");

    const contributionsPath = join(tempDir, ".forgecraft", "contributions.json");
    expect(existsSync(contributionsPath)).toBe(true);
    const saved = JSON.parse(readFileSync(contributionsPath, "utf-8")) as unknown[];
