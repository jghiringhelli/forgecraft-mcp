...
export async function convertExistingHandler(
  ...
  text += `---\n_Run \`npx forgecraft-mcp scaffold . --dry-run\` to preview what would be generated._`;
...
function buildMigrationPlan(
  ...
  if (missingChecks.has("instruction_file_exists")) {
    foundationSteps.push("Generate instruction files: `npx forgecraft-mcp generate .`");
  }
  ...
  if (missingChecks.has("hooks_installed")) {
    qualitySteps.push("Install quality gate hooks: `npx forgecraft-mcp scaffold .`");
  }
  ...
  if (missingChecks.has("tech_spec_exists")) {
    docSteps.push("Create docs/TechSpec.md — run `npx forgecraft-mcp list` for NFR reference sections");
  }
