...
<!-- Conduit RealWorld API — GS Treatment-v2 Condition (updated GS artifacts) -->
<!-- Generated by ForgeCraft `setup_project --tags UNIVERSAL,API` then adapted for experiment -->
...
## Project Identity
- **Repo**: github.com/jghiginghelli/forgecraft-mcp (experiment: `experiments/treatment-v2/output/`)
...
3. **`.husky/pre-commit`** — tsc + lint + test gate
4. **`.husky/commit-msg`** — commitlint gate
5. **`commitlint.config.js`** — conventional commits config
6. **`.github/workflows/ci.yml`** — full CI with mutation gate (`npx stryker run`)
