...
# --no-warn-ignored: a staged file that eslint config ignores (e.g. tests/**)
# must not emit a "File ignored" warning that trips --max-warnings=0.
if ! npx eslint --max-warnings=0 --no-warn-ignored $FILES 2>&1; then
